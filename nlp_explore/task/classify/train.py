...
x_train, y_train, x_test, y_test = preprocess(data_helper)
...
with tf.Graph().as_default():
    ...
    with sess.as_default():
        ...
        accuracy_summary=tf.summary.scalar('accuracy',textcnn.accuracy)
        ...
        def train_step(x_batch, y_batch):
            feed_dict={textcnn.input_x:x_batch,
                   textcnn.input_y:y_batch,
                   textcnn.dropout_keep_prob:config.model.dropout_keep_prob}
            _,step,summaries,loss,accuracy=sess.run([train_op,global_step,train_summary_op,textcnn.loss,textcnn.accuracy],
                                                         feed_dict=feed_dict)
        ...
        def evl_step():
            ...
            step, summaries, loss, accuracy = sess.run([global_step, dev_summary_op, textcnn.loss, textcnn.accuracy],
                                                       feed_dict=feed_dict)
            nowtime = datetime.datetime.now().isoformat()
            print('{}: step {}, loss {:g}, accuracy {:g}'.format(nowtime, step, loss, accuracy))
            dev_summary_writer.add_summary(summaries, step)

        batches = train_data.batch_iter(list(zip(train_x,train_y)),config.batch_size,
